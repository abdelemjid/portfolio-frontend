...
const About = () => {
  ...
  const {
    ...
    isError: isAchError,
    error: achError,
  } = useQuery({ queryKey: ["achievement"], queryFn: api.getAchievements });

  useEffect(() => {
    if (!isLoading && isError && error) {
      toast.error("About Content Error", {
        description: "Error fetching 'About' content!",
      });
    }
  }, [isError, isLoading, error]);

  useEffect(() => {
    if (!isAchLoading && isAchError && achError) {
      toast.error("Achievements Error", {
        description: "Error fetching achievements!",
      });
    }
  }, [isAchLoading, isAchError, achError]);
